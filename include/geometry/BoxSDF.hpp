...
class BoxSDF : public SDF
{
    ...
    virtual double evaluate(const Eigen::Vector3d& x) const override
    {
        ...
        const Eigen::Vector3d q = x_body.cwiseAbs() - 0.5*_box->size();
        ...
        const double dist_when_outside = std::sqrt(q0*q0 + q1*q1 + q2*q2);
        // distance of x to the surface of the box - evaluates to 0 when x is outside the box
        const double dist_when_inside = std::min(max_q, 0.0);
    ...
    virtual Eigen::Vector3d gradient(const Eigen::Vector3d& x) const override
    {
        ...
        const Eigen::Vector3d grad_when_outside = Eigen::Vector3d({sign0*q0, sign1*q1, sign2*q2}).normalized();
        ...
        const Eigen::Vector3d grad_when_inside(sign0*static_cast<double>(q[0]==min_dist), sign1*static_cast<double>(q[1]==min_dist), sign2*static_cast<double>(q[2]==min_dist));
        const Eigen::Vector3d grad = grad_when_outside + grad_when_inside;
