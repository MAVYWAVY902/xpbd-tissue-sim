...
class MeshGPUResource : public HostReadableGPUResource, public HostWritableGPUResource
{
    public:
    explicit MeshGPUResource(Geometry::Mesh* mesh)
        : _mesh(mesh)
    {
    }
    ...
    {
        CHECK_CUDA_ERROR(cudaMemcpy(_mesh->vertices().data(), _d_vertices, _vertices_size, cudaMemcpyDeviceToHost));
    }
