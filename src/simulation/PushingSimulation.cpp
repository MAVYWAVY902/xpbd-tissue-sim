...
void PushingSimulation::setup()
{
    ...
    // Add knife to collision scene manually (graphics_only=true keeps mesh stable).
    // Standard CollisionScene handles collision via Frank-Wolfe + SDF 256³ gradient.
    _collision_scene->addObject(_cursor);
    std::cout << "[PushingSimulation] Knife collision: standard CollisionScene (SDF 256^3)." << std::endl;
...
void PushingSimulation::notifyMouseMoved(double x, double y)
{
    ...
    if (_keys_held.count(SimulationInput::Key::SPACE) && _keys_held.at(SimulationInput::Key::SPACE) > 0)
    {
        ...
        else
        {
            ...
            const Real base_scaling = _tool_radius / 400.0;
...
void PushingSimulation::notifyMouseScrolled(double dx, double dy)
{
    ...
    if (_keys_held.count(SimulationInput::Key::SPACE) && _keys_held.at(SimulationInput::Key::SPACE) > 0)
    {
        ...
        {
            const Real base_scaling = _tool_radius / 12.0;  // reduced scroll sensitivity
...
void PushingSimulation::_timeStep()
{
    ...
    // Knife collision handled by standard CollisionScene (Frank-Wolfe + SDF 256³).
    // No custom post-solve projection needed.
}
