...
Geometry::TetMesh MeshUtils::loadTetMeshFromGmshFile(const std::string& filename)
{
    ...
    for(auto e : entities) {
        ...
        for (unsigned i = 0; i < nodeTags.size(); i++)
        {
            ...
            tag2idx_buffer.emplace_back(static_cast<int>(nodeTags[i]),
                                        static_cast<int>(vert_offset + i));
        }
    ...
    // NEW: persist the tag->index mapping into the mesh so Simulation can query tags directly.
    {
        ...
        mp.clear();
        mp.reserve(tag2idx_buffer.size());
        for (const auto& pr : tag2idx_buffer) {
            // pr.first  = gmsh node tag (global)
            // pr.second = final vertex column index in 'vertices'
...
void MeshUtils::createTissueBlock(const std::string& filename, const Real length, const Real width, const Real height, const int num_low_res_subdivisions, const int high_res_multiplier)
{
    ...
    // write vertices (verts is std::vector<Vec3r>, where Vec3r = Eigen::Matrix<Real,3,1>)
    ...
    // write faces (faces is std::vector<Eigen::Vector3i>), OBJ indices are 1-based
